...
const Board: React.FC<BoardProps> = ({
  ...
  const [dragUI, setDragUI] = useState<DragUIState>({
    overColumnId: null,
    insertionIndex: -1
  });
  ...
  const handleImportChange: React.ChangeEventHandler<HTMLInputElement> =
    useCallback(
      (e) => {
  ...
  const onDragOverColumn = useCallback(
    (colId: string) => (e: React.DragEvent<HTMLDivElement>) => {
      if (!dragState) return;
      e.preventDefault();
      const tasksUl = e.currentTarget.querySelector("ul.attractions");
      if (!tasksUl) return;
      const taskEls = Array.from(tasksUl.children).filter((el) =>
        el.classList.contains("attraction")
      ) as HTMLLIElement[];
      ...
      setCities((prev) => {
        const updated = prev.map((city) => ({
          ...
          columns: city.columns.map((col) => {
            if (col.id !== columnId) return col;
            const tasks = col.tasks.map((t, i) =>
              i === index ? { ...t, workingHours: newHours || undefined } : t
            );
            return { ...col, tasks };
          })
        }));
      ...
      setCities((prev) => {
        const updated = prev.map((city) => ({
          ...
          columns: city.columns.map((col) => {
            if (col.id !== columnId) return col;
            const tasks = col.tasks.map((t, i) =>
              i === index ? { ...t, VisitTime: newVisit || "" } : t
            );
            return { ...col, tasks };
          })
        }));
      ...
      setCities((prev) => {
        const updated = prev.map((city) => ({
          ...
          columns: city.columns.map((col) => {
            if (col.id !== columnId) return col;
            const tasks = col.tasks.map((t, i) =>
              i === index ? { ...t, mustVisit: !t.mustVisit } : t
            );
            return { ...col, tasks };
          })
        }));
  ...
  const upsertAttractions = useCallback(
    (columnId: string, newAttractions: Attraction[]) => {
      if (readOnly) return;
      setCities((prev) =>
        prev.map((city) => ({
          ...city,
    ...
    <div className="attraction-board-wrapper">
      <div className="board-toolbar">
        ...
        <button
          ...
          title="Download current board as JSON"
        >
          Save JSON
        </button>
        <button
          ...
          title="Load board from JSON file"
        >
          Load JSON
        </button>
        <input
        ...
        >
          <label>
            Countrywide
            <select
              value={filters.isCountrywide}
              onChange={updateFilter("isCountrywide")}
            >
          ...
          <label>
            Must Visit
            <select
              value={filters.mustVisit}
              onChange={updateFilter("mustVisit")}
            >
              <option value="any">Any</option>
          ...
          <label>
            Traditional
            <select
              value={filters.isTraditional}
              onChange={updateFilter("isTraditional")}
            >
              <option value="any">Any</option>
      ...
      {cities.map((city) => {
        ...
        const excludedColumn = city.columns.find(
          (col) => col.title === "Excluded Attractions"
        );
        const nonExcludedColumns = city.columns.filter(
          (col) => col.title !== "Excluded Attractions"
        );
        const allInExcluded =
          nonExcludedColumns.every((col) => col.tasks.length === 0) &&
          excludedColumn &&
          excludedColumn.tasks.length > 0;
        ...
        const topAndSecondaryTasks = city.columns
          .filter(
            (col) =>
              col.title === "Top Attractions" || col.title === "Secondary Spots"
          )
          .flatMap((col) => col.tasks);
        const allTopAndSecondaryPlanned =
          isCollapsed &&
          ...
          >
            <div className="city-header">
              <button
                ...
                aria-label={
                  isCollapsed ? `Expand ${city.name}` : `Collapse ${city.name}`
                }
