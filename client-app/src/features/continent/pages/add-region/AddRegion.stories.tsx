...
const withServer =
  (saveRegionStatus?: number): Decorator =>
  (Story) => {
    server?.shutdown();
    server = makeServer({ saveRegionStatus });
    return <Story />;
  };
...
export const ShowsInlineNameConflictWhenRegionAlreadyExists: Story = {
  decorators: [withServer(409)],
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const user = setupUser();
    const field = regionNameField(canvasElement);
    ...
    expect(
      await canvas.findByText("A region with this name already exists.")
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(
        canvas.queryByText("A region with this name already exists.")
