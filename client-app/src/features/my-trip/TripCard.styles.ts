import { IButtonStyles, IPalette, mergeStyleSets, useTheme } from "@fluentui/react";
import { TripStatus } from "./domain/Trip.types";

const TRIP_STATUS_COLORS: Record<TripStatus, (palette: IPalette) => { bg: string; text: string }> = {
  active: (palette) => ({ bg: palette.greenLight, text: palette.greenDark }),
  past: () => ({ bg: "#fff3e0", text: "#e65100" }),
  archived: (palette) => ({ bg: palette.neutralLighter, text: palette.neutralSecondary })
};
