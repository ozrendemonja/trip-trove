...
test('Display the edit dialog prepopulated with existing data when the edit button in the name column is clicked', async ({ page }) => {
  await page.getByRole('button', { name: "Change value for Australia" }).click();

  await expect(page).toHaveScreenshot();
});

test('Update button is disabled and error message is shown when added name is empty', async ({ page }) => {
  await page.getByRole('button', { name: "Change value for Australia" }).click();
...
test('Update button is disabled and error message is shown when added name is invalid', async ({ page }) => {
  await page.getByRole('button', { name: "Change value for Australia" }).click();
