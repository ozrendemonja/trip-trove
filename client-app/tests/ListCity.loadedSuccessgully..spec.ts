...
test("Select city when clicked on the check icon", async ({ page }) => {
  await page
...
test("Select city when click on the row", async ({ page }) => {
  await page
    .getByRole("gridcell", { name: "Change city name for Kaunas" })
    .click();
...
test("All delete menu buttons are enabled when open it", async ({ page }) => {
  await page
    .getByRole("gridcell", { name: "Change city name for Kaunas" })
    .click();
  await page.getByRole("menuitem", { name: "Delete city" }).click();
...
test("List should not contain previously deleted element when delete menu closes", async ({
  ...
  await expect(
    page
      .locator('div[data-selection-index="2"]')
      .getByRole("gridcell", { name: "Change city name for Monaco" })
  ).toHaveCount(1);
